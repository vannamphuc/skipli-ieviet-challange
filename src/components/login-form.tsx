...
import type { LoginSchemaType, OtpSchemaType } from '@/schemas/auth'
...
export function LoginForm({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  const emailForm = useForm<LoginSchemaType>({
  ...
  const onEmailSubmit = (data: LoginSchemaType) => {
    ...
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card className='border-border/50 shadow-sm'>
        <CardHeader className='text-center'>
          ...
          <CardDescription>
            {step === 'email' ? 'Enter your email to receive a login code' : `We've sent a 6-digit code to ${email}`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {step === 'email' ? (
